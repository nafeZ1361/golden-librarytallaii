...
def _ensure_time_column(df: pd.DataFrame) -> pd.DataFrame:
    """
    تضمین می‌کند که DataFrame یک ستون قابل‌استفاده به نام 'time' دارد.
    اگر ستون 'time' وجود نداشته باشد، از index یا از ستون‌های رایج دیگر
    (aligned_time / datetime / date) می‌سازد. در نبود هر کدام، از یک ایندکس عددی استفاده می‌کند.
    """
    df = df.copy()
